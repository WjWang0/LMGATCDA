...
class MAGNALayer(nn.Module):
    def __init__(self,
        ...
        self.graph_norm = STDLayerNorm(num_features=in_feats)  # entity feature normalization
        self.feed_forward = PositionwiseFeedForward(model_dim=self._out_feats, d_hidden=4 * self._out_feats)  # entity feed forward
        self.ff_norm = STDLayerNorm(num_features=self._out_feats)  # entity feed forward normalization
        self.reset_parameters()
    ...
    def forward(self, graph: DGLGraph, features, drop_edge_ids=None):
        ...
        if self.layer_norm:
            ...
            h = features
        if self.head_tail_shared:
            feat = self.fc(h).view(-1, self._num_heads, self._att_dim)
    ...
    def topk_attention(self, graph: DGLGraph):
        ...
        def topk_attn_reduce_func(nodes):
            ...
            if topk > neighbor_num:
                topk = neighbor_num
            topk_atts, _ = torch.topk(attentions, k=topk, dim=1) #  返回 列表中最大的n个值
...
class STDLayerNorm(nn.Module):
    ...
    def forward(self, x):
        mean = x.mean(dim=-1, keepdim=True)  # 按行求平均
        std = x.std(dim=-1, keepdim=True)  # std标准差
