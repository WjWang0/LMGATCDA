...
class PosNegEdgesGenerator(object):
    ...
    def __call__(self, split_type):
        ...
        if split_type == 'train':
            # Adding self loop in train avoids sampling the source node itself.#在序列中添加自环可以避免对源节点本身进行采样。
...
class SEALSampler(object):
    ...
    def __init__(self, graph, device, hop=1, num_workers=32, print_fn=print):
        self.graph = graph
        self.hop = hop
    ...
    def sample_subgraph(self, target_nodes, node_label = 'hop'):
        ...
        if node_label == 'drnl':  # DRNL
            ...
            z = de_node_labeling(subgraph, u_id, v_id).to(self.device)
        elif node_label == 'de+':  # 维度bug 不可用
            z = de_plus_node_labeling(subgraph, u_id, v_id).to(self.device)
        elif node_label == 'degree':  # this is technically not a valid labeling trick
            z = torch.tensor(subgraph.adj().to_dense().numpy().sum(axis=0)).squeeze(0)
            z[z > 100] = 100  # limit the maximum label to 100
        else:
            z = torch.zeros(len(subgraph.num_nodes()), dtype=torch.long)
        #drnl
...
class SEALData(object):
    ...
    def __init__(self,args, g, split_edge, device, hop=1, neg_samples=1, subsample_ratio=1, prefix=None, save_dir=None,
    ...
    def __call__(self, split_type):
        ...
        if osp.exists(path):
            ...
            edges, labels = self.generator(split_type)  # 正负边生成
